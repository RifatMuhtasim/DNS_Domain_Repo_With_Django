...
def cart(request):
        if request.method == 'POST':
                ...
                return render(request, 'CheckoutMoney/Cart.html', {'DnsName': DnsName})
...
def charge(request):
        if request.method == 'POST':
                ...
                return redirect(reverse('DnsCongratulations', args =[DnsName]))

        else:
                return redirect(reverse('DnsCongratulations'))